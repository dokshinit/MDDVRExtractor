...
final class ExtSplitPaneDividerPainter extends AbstractRegionPainter {
    ...
    String base = "extBase"; //"nimbusBlueGrey";
    Color baseColor = UIManager.getColor(base);
    
    private Color color1 = decodeColor(base, 0.0f, -0.017358616f, -0.11372548f, 0);
    private Color color2 = decodeColor(base, 0.055555582f, -0.102396235f, 0.21960783f, 0);
    private Color color3 = decodeColor(base, 0.0f, -0.07016757f, 0.12941176f, 0);
    private Color color4 = decodeColor("nimbusFocus", 0.0f, 0.0f, 0.0f, 0);
    private Color color5 = decodeColor(baseColor, Color.WHITE, 0.3f); //decodeColor(base, 0.0f, -0.110526316f, 0.25490195f, 0);
    private Color color6 = decodeColor(baseColor, Color.WHITE, 0.5f); //decodeColor(base, 0.0f, -0.048026316f, 0.307843137f, 0);
    private Color color7 = decodeColor(baseColor, Color.WHITE, 0.7f); //decodeColor(base, 0.0055555105f, -0.06970999f, 0.51568626f, 0);
    private Color color8 = decodeColor(baseColor, Color.WHITE, 0.9f); //decodeColor(base, 0.0f, -0.06704806f, 0.36666666f, 0);
    private Color color9 = decodeColor(base, 0.0f, -0.019617222f, -0.09803921f, 0);
    private Color color10 = decodeColor(base, 0.004273474f, -0.03790062f, -0.043137252f, 0);
    private Color color11 = decodeColor(base, -0.111111104f, -0.106573746f, 0.24705881f, 0);
    private Color color12 = decodeColor(base, 0.0f, -0.049301825f, 0.02352941f, 0);
    private Color color13 = decodeColor(base, -0.006944418f, -0.07399663f, 0.11372548f, 0);
    private Color color14 = decodeColor(base, -0.018518567f, -0.06998578f, 0.12549019f, 0);
    private Color color15 = decodeColor(base, 0.0f, -0.050526317f, 0.039215684f, 0);
    //Array of current component colors, updated in each paint call
    private Object[] componentColors;

    public ExtSplitPaneDividerPainter(PaintContext ctx, int state) {
        super();
        this.state = state;
        this.ctx = ctx;
    }
    ...
    private void paintForegroundEnabled(Graphics2D g) {
        roundRect = decodeRoundRect1(); // Обводка.
        g.setPaint(decodeGradient3(roundRect));
        g.fill(roundRect);
        roundRect = decodeRoundRect2(); // Фон.
