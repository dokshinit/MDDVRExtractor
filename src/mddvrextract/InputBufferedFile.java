...
/**
 * Файл для буферизованного чтения с кэшированием данных.
 *
 * Позволяет возвращаться назад и читать в пределах размера кэша без обращения к
 * диску. Справедливо для любого направления движения, от начала к концу или
 * наоборот.
 *
...
public class InputBufferedFile {
    ...
    /**
     * Размер глубины буфера при последовательных смещениях. Т.е. при смещении
     * буфера пред.данные в указанном размере остаются в буфере. Для оптимизации
     * обащений к диску.
     */
    ...
    /**
     * Текущая логическая позиция в файле (не соответствует реальной, реального
     * позиционирвания не происходит!)
     */
     ...
     * смещении буфера. Например: 100кб буфер и 10кб кэш - при чтении >100кб
     * буфер сдвинется на (100-10)кб при этом последние 10кб буфера сдвинутся в
     * начало и 90кб дочитаются следом за ними.
     *
     * Т.е. смысл в том, чтобы если возникнет нужда вернуть указатель назад и
     * считать инфу заново, то в пределах размера кэша это не вызовет обращения
     * к файлу - будет читаться из буфера.
     *
     * @param desc Дескриптор файла-источника.
    ...
    /**
     * Возвращает байт-буфер для внешних операций с буфером. Например -
     * считывание данных напрямую из буфера (безопасно в пределах кэша т.к.
     * данные заведомо есть!).
     *
     * @return Байт-буфер.
     */
    ...
    /**
     * Актуализирует содержимое буфера согласно текущей позиции. Если необходимо
     * - происходит сдвиг буфера и обновление недостающих данных.
     *
     * @throws IOException
     */
    ...
    /**
     * Буферизированное считывание данных в массив с нулевого индекса с
     * тек.позиции.
     *
     * @param ba Приёмник.
     * @param size Размер считываемого блока.
    ...
    /**
     * Чтение byte из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
    ...
    /**
     * Чтение short из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
    ...
    /**
     * Чтение int из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
    ...
    /**
     * Чтение long из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
    ...
    /**
     * Чтение float из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
    ...
    /**
     * Чтение double из файла с указанной позиции (буферизированно). Указатель
     * смещается.
     *
     * @param pos Позиция.
     * @return Считанное значение.
