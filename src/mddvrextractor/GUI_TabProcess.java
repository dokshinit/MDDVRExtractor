...
import mddvrextractor.gui.GroupBorder;
import mddvrextractor.gui.JVScrolledPanel;
import mddvrextractor.gui.GUI;
import mddvrextractor.gui.JDateTimeField;
import mddvrextractor.gui.RoundPanel;
import mddvrextractor.FFMpeg.Cmd;
import mddvrextractor.FFMpeg.FFCodec;
import mddvrextractor.gui.JExtComboBox.ExtItem;
...
public final class GUI_TabProcess extends JPanel implements ActionListener {
    ...
    public void createUI() {
        ...
        if (checkExpert.isSelected()) { // Режим расширенных настроек.
            ...
            p2.add(comboVideoFormat, "left, spanx, wmax 600, wrap");
            ...
            p3.add(comboAudioFormat, "left, spanx, wmax 600, wrap");
            p3.add(labelNote, "skip, spanx, wrap");
    ...
    private Cmd getVideoOptions() {
        ...
        if (checkExpert.isSelected()) { // Режим эксперта.
            ...
            if (i.id == 1000) {
                ...
                if (i.id != 0) {
                    s.add("-sameq"); // Без потери качества! (для copy не применяем!)
                }
            }
