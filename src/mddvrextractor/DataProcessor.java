...
public class DataProcessor {
    ...
    private static void finalMake() throws FatalException {
        ...
        // Вообще же, подсовывать таким способом - это гарантированные ошибки,
        // если аудио поток дискретный, нужна более адекватная обработка.
        if (isAudio && isAudioTemp && audioProcessSize > 0) {
    ...
    private static void startFFMpegProcess() throws FatalException {
        ...
        // Настройки приёмника.
        Cmd vcmd = new Cmd("-f", "h264", "-r", sfps, "-i", "-");
        ...
        if (isAudio) {
            try {
                processAudio = acmd.startProcess();
            } catch (IOException ex) {
